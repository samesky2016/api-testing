...
Integration Testing Skill 是面向 Claude Code 的 API 集成测试自动化技能，通过 `/integration-test` 斜杠命令或 `@integration-testing` agent 调用。
...
```bash
unzip integration-testing_v9.zip
cp -r test_skills_v9/.claude  /your-project/
```
...
.claude/
├── commands/
│   └── integration-test.md      # /integration-test 斜杠命令
├── agents/
│   └── integration-testing.md   # @integration-testing agent 定义
└── skills/test-skills/
    ├── SKILL.md                 # 技能索引（allowed-tools / invocation）
    ├── scripts/
    │   ├── discover_cases.py    # 用例发现 / 生成（1098 行）
    │   ├── run_test.py          # 用例执行 + HTTP 留痕（493 行）
    │   ├── gen_reports.py       # 报告生成 MD / HTML / JSON（845 行）
    │   └── record.sh            # 留痕编排 Shell（118 行）
    ├── assets/
    │   └── http_client.py       # 降级 HTTP 执行器（requests 库）
    └── references/
        ├── integration-testing.md   # 多语言框架示例（Supertest / pytest / REST Assured）
        ├── audit-trail.md           # 留痕字段语义说明
        └── case-generation.md       # 用例生成规范
```
...
/integration-test http://localhost:8080 --generate-all

# 指定 Knife4j 服务（推荐，服务运行时）
/integration-test http://localhost:8080 --knife4j-url http://localhost:8080 --generate-all

# 有认证 token 时（内部业务系统必须加 --force-auth）
/integration-test http://localhost:8080 --knife4j-url http://localhost:8080 \
  --knife4j-token "Bearer eyJ..." --force-auth --generate-all

# 使用本地 Swagger 文件
/integration-test http://localhost:8080 --api-doc docs/test/test-data/swagger.json --generate-all
...
```
步骤 1  初始化留痕目录
        bash .claude/skills/test-skills/scripts/record.sh init
...
**① 所有用例必须执行**
发现多少用例执行多少，不存在 SKIP 路径。`run_test.py` 已移除 `--skip` 参数，无任何跳过路径。若接口不可达，`actual_status=0`，断言强制 FAIL，如实记录。
...
/integration-test http://localhost:8080 \
...
检查 `.claude/agents/integration-testing.md` 的 YAML frontmatter 是否完整，必须同时有开头和结尾的 `---`：
...
正确调用方式：
```bash
/integration-test http://... --knife4j-url http://... \
...
```bash
# 延长到 10 分钟
/integration-test http://localhost:8080 --generate-all --suite-timeout 600
```
